...
class Player:
    ...
    def __init__(
        self, model_name: str, player_name: str, role: Role, language: str = "English"
    ):
    ...
    def __str__(self) -> str:
        return f"{self.player_name} ({self.role.value}) [Model: {self._model_name}]"

    def _find_target_player(
        self, target_name: str, all_players: list, exclude_mafia: bool = False
    ) -> Optional["Player"]:
    ...
    def generate_prompt(
        ...
        if self.role == Role.MAFIA and mafia_members:
            ...
            mafia_list = (
                ", ".join(mafia_names)
                if mafia_names
                else "None (you are the only Mafia left)"
            )
            ...
            template = PROMPT_TEMPLATES[language].get(
                self.role, PROMPT_TEMPLATES[language][Role.VILLAGER]
            )
    ...
    def get_response(self, prompt: str) -> str:
        ...
        response = get_llm_response(self._model_name, prompt)
    ...
    def parse_night_action(
        self, response: str, all_players: list
